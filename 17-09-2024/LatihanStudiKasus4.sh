...
function showBudget() {
    ...
    if [ $budgetRemaining -lt 0 ]; then
        echo "Biaya pertunjukan melebihi anggaran sebanyak ${budgetRemaining#-}, jadi anggaran tidak mencukupi" # if budgetRemaining less than 0
    else
        echo "Biaya pertunjukan dicukupi oleh anggaran"
    fi
}
